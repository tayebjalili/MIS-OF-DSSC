            ...
            <div class="card">
                <div class="card-body">
                    <div class="header-toolbar">
                        <h5 class="card-title mb-0" style="font-size: 14px;">{{ __('messages.mental_health_specialist') }}</h5>

                        <div class="search-box">
                            <form method="GET" action="{{ url('panel/health') }}" class="d-flex w-100">
                        ...
                        <div class="utility-buttons">
                            ...
                            <button class="filter-toggle-btn {{ request('universitym') ? 'has-filters' : '' }}" id="filterToggleBtn">
                            ...
                            <div class="notes-container">
                                ...
                                <div class="notes-dropdown">
                                    ...
                                    <div class="note-input-group">
                                        <input type="text" id="noteInput" class="form-control note-input"
                                            placeholder="{{ __('messages.Type your note...') }}">
                            ...
                            <form action="{{ route('backup.page', ['page' => 'health']) }}" method="GET">
                                <button type="submit" class="btn backup-btn">
                                    <i class="bi bi-box-arrow-down"></i> Backup
                                </button>
                            </form>

                            @hasPermission('Add Mental Health Specialist')
                            <a class="btn btn-success" href="{{ url('panel/health/add') }}">
                ...
                'order' => (request('sort_by') === 'id' && request('order') === 'asc') ? 'desc' : 'asc'
                                    ...
                                    <th style="width: 120px;">
                                        <a href="{{ route(request()->route()->getName(), array_merge(request()->all(), [
                'sort_by' => 'job_title',
                'order' => (request('sort_by') === 'job_title' && request('order') === 'asc') ? 'desc' : 'asc'
            ])) }}" style="text-decoration: none; color: inherit; display: inline-flex; align-items: center; gap: 4px;">
                                            {{ __('messages.job_title') }}
                                            <span style="font-size: 12px; color: #888;">
                                                @if(request('sort_by') === 'job_title')
                                                <span style="color: #6200ee; font-weight: bold;">
                                                    {{ request('order') === 'asc' ? '▲' : '▼' }}
                                                </span>
                                                @else
                                    ...
                                    <th style="width: 120px;">
                                        <a href="{{ route(request()->route()->getName(), array_merge(request()->all(), [
                'sort_by' => 'universitym',
                'order' => (request('sort_by') === 'universitym' && request('order') === 'asc') ? 'desc' : 'asc'
            ])) }}" style="text-decoration: none; color: inherit; display: inline-flex; align-items: center; gap: 4px;">
                                            {{ __('messages.universitym') }}
                                            <span style="font-size: 12px; color: #888;">
                                                @if(request('sort_by') === 'universitym')
                                                <span style="color: #6200ee; font-weight: bold;">
                                                    {{ request('order') === 'asc' ? '▲' : '▼' }}
                                                </span>
                                                @else
                                    ...
                                    <th style="width: 120px;">
                                        <a href="{{ route(request()->route()->getName(), array_merge(request()->all(), [
                'sort_by' => 'problem',
                'order' => (request('sort_by') === 'problem' && request('order') === 'asc') ? 'desc' : 'asc'
            ])) }}" style="text-decoration: none; color: inherit; display: inline-flex; align-items: center; gap: 4px;">
                                            {{ __('messages.problem') }}
                                            <span style="font-size: 12px; color: #888;">
                                                @if(request('sort_by') === 'problem')
                                                <span style="color: #6200ee; font-weight: bold;">
                                                    {{ request('order') === 'asc' ? '▲' : '▼' }}
                                                </span>
                                                @else
                                    ...
                                    <th style="width: 120px;">
                                        <a href="{{ route(request()->route()->getName(), array_merge(request()->all(), [
                'sort_by' => 'instructor',
                'order' => (request('sort_by') === 'instructor' && request('order') === 'asc') ? 'desc' : 'asc'
            ])) }}" style="text-decoration: none; color: inherit; display: inline-flex; align-items: center; gap: 4px;">
                                            {{ __('messages.instructor') }}
                                            <span style="font-size: 12px; color: #888;">
                                                @if(request('sort_by') === 'instructor')
                                                <span style="color: #6200ee; font-weight: bold;">
                                                    {{ request('order') === 'asc' ? '▲' : '▼' }}
                                                </span>
                                                @else
                                    ...
                                    <th style="width: 100px;">
                                        <a href="{{ route(request()->route()->getName(), array_merge(request()->all(), [
                'sort_by' => 'duration',
                'order' => (request('sort_by') === 'duration' && request('order') === 'asc') ? 'desc' : 'asc'
            ])) }}" style="text-decoration: none; color: inherit; display: inline-flex; align-items: center; gap: 4px;">
                                    ...
                                    <th style="width: 100px;">
                                        <a href="{{ route(request()->route()->getName(), array_merge(request()->all(), [
                'sort_by' => 'result',
                'order' => (request('sort_by') === 'result' && request('order') === 'asc') ? 'desc' : 'asc'
            ])) }}" style="text-decoration: none; color: inherit; display: inline-flex; align-items: center; gap: 4px;">
                                            {{ __('messages.result') }}
                                            <span style="font-size: 12px; color: #888;">
                                    ...
                                    <th style="width: 120px;">
                                        <a href="{{ route(request()->route()->getName(), array_merge(request()->all(), [
                'sort_by' => 'patient_intro',
                'order' => (request('sort_by') === 'patient_intro' && request('order') === 'asc') ? 'desc' : 'asc'
            ])) }}" style="text-decoration: none; color: inherit; display: inline-flex; align-items: center; gap: 4px;">
                                            {{ __('messages.patient_intro') }}
                                            <span style="font-size: 12px; color: #888;">
                                                @if(request('sort_by') === 'patient_intro')
                                                <span style="color: #6200ee; font-weight: bold;">
                                                    {{ request('order') === 'asc' ? '▲' : '▼' }}
                                                </span>
                                                @else
                                    ...
                                    <th style="width: 100px;">
                                        <a href="{{ route(request()->route()->getName(), array_merge(request()->all(), [
                'sort_by' => 'education',
                'order' => (request('sort_by') === 'education' && request('order') === 'asc') ? 'desc' : 'asc'
            ])) }}" style="text-decoration: none; color: inherit; display: inline-flex; align-items: center; gap: 4px;">
                                            {{ __('messages.education') }}
                                            <span style="font-size: 12px; color: #888;">
                                                @if(request('sort_by') === 'education')
                                                <span style="color: #6200ee; font-weight: bold;">
                                                    {{ request('order') === 'asc' ? '▲' : '▼' }}
                                                </span>
                                                @else
                                    ...
                                    @hasPermission('Edit Mental Health Specialist')
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($value->file)
                                        <a href="{{ url('panel/health/show/' . $value->id) }}"
                                            class="btn btn-primary btn-sm" target="_blank">{{ __('messages.show') }}</a>
                                        @else
                                            ...
                                            <ul class="dropdown-menu">
                                                ...
                                                <li>
                                                    <a class="dropdown-item"
                                                        href="{{ url('panel/health/edit/' . $value->id) }}">
                                                        <i class="bi bi-pencil"></i> {{ __('messages.edit') }}
                                                    </a>
                                                </li>
                                                @endhasPermission
                                                @hasPermission('Delete Mental Health Specialist')
                                                <li>
                                                    <a class="dropdown-item text-danger"
                                                        href="{{ url('panel/health/delete/' . $value->id) }}"
...
<div class="filter-modal-overlay" id="filterModal">
    <div class="filter-modal">
        ...
        <div class="filter-modal-body">
            <form method="GET" action="{{ url('panel/health') }}" id="filterForm">
                <div class="filter-row">
                    <!-- University Filter -->
                    <div class="filter-group">
                        <label class="filter-label">@lang('messages.universitym')</label>
                        <select name="universitym" class="form-control filter-select">
                            <option value="">@lang('messages.all_universities')</option>
                            @foreach($universities as $university)
                                <option value="{{ $university }}" {{ request('universitym') == $university ? 'selected' : '' }}>
                                    {{ $university }}
                                </option>
                            @endforeach
                        </select>
        ...
        <div class="filter-modal-footer">
            <div class="filter-results-badge">
                ...
                <span id="filterResultsText">
                    @if(request('universitym'))
                        @lang('messages.filtered_by_university')
                    @else
...
<script>
    ...
    // Notes functionality
    const storageKey = 'temporaryNotes_health'; // Unique key for this page
    let noteIdCounter = (() => {
