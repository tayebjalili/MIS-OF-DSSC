            ...
            <div class="card">
                <div class="card-body">
                    <div class="header-toolbar">
                        <h5 class="card-title mb-0" style="font-size: 14px;">{{ __('messages.specialist_of_coordination_and_university_relations') }}</h5>

                        <div class="search-box">
                            <form method="GET" action="{{ url('panel/specialist') }}" class="d-flex w-100">
                        ...
                        <div class="utility-buttons">
                            ...
                            <form action="{{ route('backup.page', ['page' => 'specialist']) }}" method="GET">
                                <button type="submit" class="btn backup-btn">
                                    <i class="bi bi-box-arrow-down"></i> Backup
                                </button>
                            </form>

                            @hasPermission('Add Specialist of Coordination and University Relations')
                            <a class="btn btn-success" href="{{ url('panel/specialist/add') }}">
                ...
                'order' => (request('sort_by') === 'activity_name' && request('order') === 'asc') ? 'desc' : 'asc'
            ])) }}"
                                            style="text-decoration: none; color: inherit; display: inline-flex; align-items: center; gap: 4px;">
                ...
                'order' => (request('sort_by') === 'activity_date' && request('order') === 'asc') ? 'desc' : 'asc'
            ])) }}"
                                            style="text-decoration: none; color: inherit; display: inline-flex; align-items: center; gap: 4px;">
                                            {{ __('messages.activity_date') }}
                                            <span style="font-size: 12px; color: #888;">
                                                @if(request('sort_by') === 'activity_date')
                                                <span style="color: #6200ee; font-weight: bold;">
                                                    {{ request('order') === 'asc' ? '▲' : '▼' }}
                                                </span>
                                                @else
                ...
                'order' => (request('sort_by') === 'description' && request('order') === 'asc') ? 'desc' : 'asc'
                                    ...
                                    <th style="width: 120px;">
                                        <a href="{{ route(request()->route()->getName(), array_merge(request()->all(), [
                'sort_by' => 'departmentt',
                'order' => (request('sort_by') === 'departmentt' && request('order') === 'asc') ? 'desc' : 'asc'
            ])) }}"
                                            style="text-decoration: none; color: inherit; display: inline-flex; align-items: center; gap: 4px;">
                                            {{ __('messages.departmentt') }}
                                            <span style="font-size: 12px; color: #888;">
                                                @if(request('sort_by') === 'departmentt')
                                                <span style="color: #6200ee; font-weight: bold;">
                                                    {{ request('order') === 'asc' ? '▲' : '▼' }}
                                                </span>
                                                @else
                                    ...
                                    @hasPermission('Edit Specialist of Coordination and University Relations')
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($value->file)
                                        <a href="{{ url('panel/specialist/show/'.$value->id) }}" class="btn btn-primary btn-sm" target="_blank">{{ __('messages.show') }}</a>
                                        @else
                                    ...
                                    <td>
                                        <div class="action-dropdown">
                                            ...
                                            <ul class="dropdown-menu">
                                                @hasPermission('Edit Specialist of Coordination and University Relations')
                                                <li>
                                                    <a class="dropdown-item" href="{{ url('panel/specialist/edit/'.$value->id) }}">
                                                        <i class="bi bi-pencil"></i> {{ __('messages.edit') }}
                                                    </a>
                                                </li>
                                                @endhasPermission
                                                @hasPermission('Delete Specialist of Coordination and University Relations')
                                                <li>
                                                    <a class="dropdown-item text-danger" href="{{ url('panel/specialist/delete/'.$value->id) }}"
...
<script>
    const storageKey = 'temporaryNotes_specialist'; // Unique key for this page
    let noteIdCounter = (() => {
