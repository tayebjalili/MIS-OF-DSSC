            ...
            <div class="card">
                <div class="card-body">
                    <div class="header-toolbar">
                        <h5 class="card-title mb-0" style="font-size: 14px;">{{ __('messages.job_placement') }}</h5>

                        <div class="search-box">
                            <form method="GET" action="{{ url('panel/coordination') }}" class="d-flex w-100">
                        ...
                        <div class="utility-buttons">
                            ...
                            <form action="{{ route('backup.page', ['page' => 'coordination']) }}" method="GET">
                                <button type="submit" class="btn backup-btn">
                                    <i class="bi bi-box-arrow-down"></i> Backup
                                </button>
                            </form>

                            @hasPermission('Add Directorate of Coordination and Student Affairs')
                            <a class="btn btn-success" href="{{ url('panel/coordination/create') }}">
                        ...
                        <table class="table table-bordered table-hover align-middle">
                            <thead class="table-light">
                                <tr>
                                    ...
                                    $columns = [
                                    'id' => __('messages.id'),
                                    'academic_institution_name' => __('messages.academic_institution_name'),
                                    'relevant_management_in_universities' => __('messages.relevant_management_in_universities'),
                                    'creative_students_intro' => __('messages.creative_students_intro'),
                                    'cV_writing' => __('messages.cV_writing'),
                                    'new_students_orientation' => __('messages.new_students_orientation'),
                                    'honor_students_encouragement' => __('messages.honor_students_encouragement'),
                                    'short_term_courses_credits' => __('messages.short_term_courses_credits'),
                                    'disabled_students' => __('messages.disabled_students'),
                                    'file' => __('messages.file'),
                                    ];
                                    @endphp
                                    @foreach ($columns as $field => $label)
                                    <th style="min-width: 150px;">
                                        <a href="{{ sortUrl($field, $sort_by, $order) }}" style="text-decoration: none; color: inherit; display: inline-flex; align-items: center; gap: 4px;">
                                            {{ $label }}
                                    ...
                                    @hasPermission('Edit Directorate of Coordination and Student Affairs')
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($field === 'file')
                                        @if($value->file)
                                        <a href="{{ url('panel/coordination/show/'.$value->id) }}" class="btn btn-primary btn-sm" target="_blank">
                                            {{ __('messages.show') }}
                                        </a>
                                        @else
                                    ...
                                    <td>
                                        <div class="action-dropdown">
                                            ...
                                            <ul class="dropdown-menu">
                                                @hasPermission('Edit Directorate of Coordination and Student Affairs')
                                                <li>
                                                    <a class="dropdown-item" href="{{url('panel/coordination/edit/'.$value->id)}}">
                                                        <i class="bi bi-pencil"></i> {{ __('messages.edit') }}
                                                    </a>
                                                </li>
                                                @endhasPermission
                                                @hasPermission('Delete Directorate of Coordination and Student Affairs')
                                                <li>
                                                    <a class="dropdown-item text-danger" href="{{url('panel/coordination/delete/'.$value->id)}}"
...
<div class="filter-modal-overlay" id="filterModal">
    <div class="filter-modal">
        ...
        <div class="filter-modal-body">
            <form method="GET" action="{{ url('panel/coordination') }}" id="filterForm">
                <div class="filter-row">
                    <!-- University Filter -->
                    <div class="filter-group">
                        <label class="filter-label">@lang('messages.academic_institution_name')</label>
                        <select name="academic_institution_name" class="form-control filter-select">
                            <option value="">@lang('messages.all_universities')</option>
                            @foreach($universities as $university)
                                <option value="{{ $university }}" {{ request('academic_institution_name') == $university ? 'selected' : '' }}>
                                    {{ $university }}
                                </option>
                            @endforeach
                        </select>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.resetFilters = function() {
        window.location.href = "{{ url('panel/coordination') }}";
    };

    // Notes functionality
    const storageKey = 'temporaryNotes_coordination';
    let noteIdCounter = (() => {
