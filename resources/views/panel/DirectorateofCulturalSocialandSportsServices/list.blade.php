            ...
            <div class="card">
                <div class="card-body">
                    <div class="header-toolbar">
                        <h5 class="card-title mb-0" style="font-size: 14px;">{{ __('messages.directorate_of_sport_social_science') }}</h5>

                        <div class="search-box">
                            <form method="GET" action="{{ url('panel/cultural') }}" class="d-flex w-100">
                        ...
                        <div class="utility-buttons">
                            ...
                            <form action="{{ route('backup.page', ['page' => 'cultural']) }}" method="GET">
                                <button type="submit" class="btn backup-btn">
                                    <i class="bi bi-box-arrow-down"></i> Backup
                                </button>
                            </form>

                            @hasPermission('Add Directorate of Cultural Social and Sports Services')
                            <a class="btn btn-success" href="{{ url('panel/cultural/add') }}">
                        ...
                        <table class="table table-bordered table-hover align-middle">
                            <thead class="table-light">
                                <tr>
                                    ...
                                    <th style="width: 150px;">
                                        <a href="{{ sortUrl('job_title', $sort_by, $order) }}">
                                    ...
                                    <th style="width: 120px;">
                                        <a href="{{ sortUrl('location', $sort_by, $order) }}">
                                    ...
                                    <th style="width: 120px;">
                                        <a href="{{ sortUrl('reports_to', $sort_by, $order) }}">
                                            {{ __('messages.reports_to') }}
                                            <span style="font-size: 12px; color: #888;">
                                                @if($sort_by === 'reports_to')
                                                <span style="color: #6200ee; font-weight: bold;">
                                                    {{ $order === 'asc' ? '▲' : '▼' }}
                                                </span>
                                                @else
                                    ...
                                    <th style="width: 200px;">
                                        <a href="{{ sortUrl('essential_notes', $sort_by, $order) }}">
                                            {{ __('messages.essential_notes') }}
                                            <span style="font-size: 12px; color: #888;">
                                                @if($sort_by === 'essential_notes')
                                                <span style="color: #6200ee; font-weight: bold;">
                                                    {{ $order === 'asc' ? '▲' : '▼' }}
                                                </span>
                                                @else
                                    ...
                                    @hasPermission('Edit Directorate of Cultural Social and Sports Services')
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{$value->essential_notes}}</td>
                                    <td>
                                        @if($value->file)
                                        <a href="{{ url('panel/cultural/show/'.$value->id)}}" class="btn btn-primary btn-sm" target="_blank">{{ __('messages.file') }}</a>
                                        @else
                                    ...
                                    <td>
                                        <div class="action-dropdown">
                                            ...
                                            <ul class="dropdown-menu">
                                                @hasPermission('Edit Directorate of Cultural Social and Sports Services')
                                                <li>
                                                    <a class="dropdown-item" href="{{url('panel/cultural/edit/'.$value->id)}}">
                                                        <i class="bi bi-pencil"></i> {{ __('messages.edit') }}
                                                    </a>
                                                </li>
                                                @endhasPermission
                                                @hasPermission('Delete Directorate of Cultural Social and Sports Services')
                                                <li>
                                                    <a class="dropdown-item text-danger" href="{{url('panel/cultural/delete/'.$value->id)}}"
...
<div class="filter-modal-overlay" id="filterModal">
    <div class="filter-modal">
        ...
        <div class="filter-modal-body">
            <form method="GET" action="{{ url('panel/cultural') }}" id="filterForm">
                <div class="filter-row">
                    <!-- Location Filter -->
                    <div class="filter-group">
                        <label class="filter-label">@lang('messages.location')</label>
                        <select name="location" class="form-control filter-select">
                            <option value="">@lang('messages.all_locations')</option>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.resetFilters = function() {
        window.location.href = "{{ url('panel/cultural') }}";
    };

    // Notes functionality
    const storageKey = 'temporaryNotes_cultural';
    let noteIdCounter = (() => {
